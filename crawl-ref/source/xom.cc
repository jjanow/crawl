...
static int _xom_random_stickable(const int HD)
{
    ...
    // weapon_rarity now gives us 1-100 (scaled from 1-10), so we divide by 10
    // to get back to the 1-10 scale for this comparison.
    do
    {
        c = random2(HD);
    }
    while (c >= ARRAYSZ(arr)
           || random2(HD) > weapon_rarity(arr[c]) / 10 && x_chance_in_y(c, HD));
