...
spret cast_putrefaction(monster* target, int pow, bool fail)
{
    ...
    const int base_duration = 5 + div_rand_round(pow, 4);
    place_cloud(CLOUD_MIASMA, target->pos(), random_range(base_duration, base_duration + 4), &you);

    // Then start a spread of fiant miasma that will become proper miasma a
    // turn later.
    const int spread_duration = 18 + div_rand_round(pow, 2);
    map_cloud_spreader_marker *marker =
    new map_cloud_spreader_marker(target->pos(), CLOUD_FAINT_MIASMA, 7,
                                        random_range(spread_duration, spread_duration + 10), 5, 2, &you);
